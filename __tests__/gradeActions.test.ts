...
// IMPORTANT: Mock database dependencies BEFORE importing gradeActions
// This prevents Jest from trying to parse mongoose/bson ES modules
...
// Mock mongoose models (empty objects since we only test pure functions)
...
// Now we can safely import the calculation functions
// Note: We're only importing the pure calculation functions, not the ones that use the database
import { calculateGPA, calculateLetterGrade } from '@/actions/gradeActions'

describe('Grade Calculations', () => {
  describe('calculateLetterGrade', () => {
    test('calculates AA for score >= 90', async () => {
      const examScores = { Midterm: 95, Final: 92 }
      const examPercentages = { Midterm: 40, Final: 60 }
      // Weighted: (95 * 0.4) + (92 * 0.6) = 38 + 55.2 = 93.2
      
      const grade = await calculateLetterGrade(examScores, examPercentages)
      expect(grade).toBe('AA')
    })

    test('calculates BA for score >= 85 and < 90', async () => {
    ...
    test('calculates BB for score >= 80 and < 85', async () => {
    ...
    test('calculates CC for score >= 70 and < 75', async () => {
    ...
    test('calculates DD for score >= 60 and < 65', async () => {
    ...
    test('calculates FF for score < 60', async () => {
    ...
    test('handles multiple exam types with custom percentages', async () => {
      const examScores = { 
        Quiz: 90, 
        Midterm: 85, 
        Final: 80 
    ...
    test('returns FF when total weight is 0', async () => {
  ...
  describe('calculateGPA', () => {
    test('calculates GPA correctly for AA and BB grades', async () => {
      const grades = [
      ...
      const gpa = await calculateGPA(grades)
      expect(gpa).toBeCloseTo(3.43, 2)
    })

    test('calculates GPA correctly for mixed grades', async () => {
      const grades = [
    ...
    test('returns 0 for empty grades array', async () => {
      const gpa = await calculateGPA([])
      expect(gpa).toBe(0)
    })

    test('handles FF grades (counts as 0 points)', async () => {
      const grades = [
        { letter_grade: 'AA', akts: 3 }, // 4.0 * 3 = 12
    ...
    test('handles courses with 0 ECTS credits', async () => {
      const grades = [
    ...
    test('returns 0 when all credits are 0', async () => {
      const grades = [
    ...
    test('handles all grade types correctly', async () => {
      const grades = [
